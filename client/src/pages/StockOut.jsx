...
const StockOut = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showToast(`${formData.quantity} units dispatched from stock!`);
  ...
  const confirmDelete = async () => {
    try {
      setDeleting(true);
      await API.delete(`/stock-out/${deleteId}`);
      setIsDeleteOpen(false);
      showToast("Stock Out record deleted and inventory restored.");
      fetchData();
    } catch (err) {
  ...
  const handleExportCSV = () => {
    ...
    const rows = [["Issued Date", "Product Name", "SKU", "Quantity Issued", "Notes"]];

    filteredRecords.forEach((r) => {
      rows.push([
        new Date(r.issuedDate || r.createdAt).toLocaleDateString(),
        `"${r.product?.name || "N/A"}"`,
    ...
    link.setAttribute("download", `stock_out_export_${new Date().toISOString().split("T")[0]}.csv`);
  ...
  const totalStockOutLogs = records.length;
  const totalUnitsDispatched = records.reduce((sum, r) => sum + (r.quantity || 0), 0);
  const recentDispatches = records.filter(
    (r) => new Date() - new Date(r.issuedDate || r.createdAt) < 30 * 24 * 60 * 60 * 1000
  ).length;
  ...
  const columns = [
    {
      ...
      render: (row) => (
        <span style={{ color: "#475569", fontSize: "14px", fontWeight: "500" }}>
          {new Date(row.issuedDate || row.createdAt).toLocaleDateString("en-GB", {
            day: "2-digit",
            month: "short",
            year: "numeric",
          })}
    ...
    },
    {
      ...
      render: (row) => (
        <span style={{ color: "#64748b", fontSize: "14px" }}>
          {row.notes || "Outgoing stock dispatch"}
        </span>
      ),
    },
    {
    ...
    <div className="fade-in">
      ...
      <div className="page-header" style={{ marginBottom: "24px" }}>
        <div>
          ...
          <p className="page-subtitle">
            Log outbound inventory dispatches to automatically deduct product stock levels.
          </p>
        </div>
        ...
        <div className="page-actions-row">
          ...
          <button onClick={handleOpenAddModal} className="btn btn-primary" style={styles.addStockOutBtn}>
            <Plus size={18} />
      ...
      {loading ? (
        ...
        <div style={styles.emptyCard} className="card">
          ...
          <h3 style={styles.emptyTitle}>No stock out records found</h3>
          <p style={styles.emptySub}>
            Record outbound inventory dispatches to track product shipments.
          </p>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            ...
            <textarea
              ...
              rows="3"
              placeholder="e.g. Customer order dispatch #802"
          ...
          <div style={styles.modalFooter}>
            ...
            <button
              ...
              disabled={submitting}
            >
              {submitting ? "Processing..." : "Add Stock Out"}
            </button>
          </div>
        </form>
      </Modal>
      ...
      <ConfirmModal
        ...
        title="Delete Stock Out Record"
        message="Are you sure you want to delete this Stock Out record? Product inventory will be restored."
        loading={deleting}
      />
    </div>
  );
